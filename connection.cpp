...
bool Connection::sendCard(const QString &card)
{
    if (card.isEmpty())
        return false;

    QByteArray c = card.toUtf8();
    QByteArray data = "CARD " + QByteArray::number(c.size()) + ' ' + c;
    return write(data) == data.size();
}

bool Connection::sendGroupOfCards(const QString &cards)
{
    if (cards.isEmpty())
        return false;

    qDebug() << cards <<"<- Iz funkcije!";

    QByteArray c = cards.toUtf8();
    QByteArray data = "GROUP " + QByteArray::number(c.size()) + ' ' + c;
    return write(data) == data.size();
}
...
void Connection::processData()
{
    ...
    case Pong:
        ...
        break;
    case Card:
        emit cardThrown(QString::fromUtf8(buffer));
        break;
    case Group:
        emit groupThrown(QString::fromUtf8(buffer));
        break;
    default:
        break;
    }
